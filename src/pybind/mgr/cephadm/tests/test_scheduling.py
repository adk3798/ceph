    ...
    if service_type == 'rgw':
        ...
        if expected and any('3049' in str(e) for e in expected):
            ...
            spec = NFSServiceSpec(service_type=service_type,
                                  ...
                                  colocation_ports=[[3049, 9588], [3050, 9589], [3051, 9590], [3052, 9591]])
        else:
...
def test_nfs_colocation_ports_validation():
    ...
    spec = NFSServiceSpec(
        ...
        port=2049,
        monitoring_port=9587,
        colocation_ports=[[3049, 9588], [4049, 9589]]
    )
    ...
    with pytest.raises(SpecValidationError) as e:
        spec = NFSServiceSpec(
            ...
            port=2049,
            monitoring_port=9587,
            colocation_ports=[[3049, 9588]]
        )
        spec.validate()
    assert "colocation_ports has 1 entries, but 3 entries are required" in str(e.value)

    # Invalid case: wrong number of ports in entry (only 1 port instead of 2)
    with pytest.raises(SpecValidationError) as e:
        spec = NFSServiceSpec(
            service_id='mynfs',
            placement=PlacementSpec(count=3),
            port=2049,
            monitoring_port=9587,
            colocation_ports=[[3049], [4049, 9589]]
        )
        spec.validate()
    assert "colocation_ports[0] has 1 ports, but 2 ports are required" in str(e.value)
